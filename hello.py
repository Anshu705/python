...
# Alwyas try to improve your code readability and maintainability by using comments and ,
# documentation to explain the code logic and functionality.
# Python is a a case sensitive programming language, which means that it treats uppercase and lowercase letters as different characters.
# Just keep in mind use comments & Make some pratice to improve your coding skills and become a better Python developer.
# Python invented by Guido van Rossum in 1991 and is now one of the most popular programming languages in the world.
# Python invertor al-ready code all logic behind the scene, so we can focus on writing the code logic and ,
...
# For it details need to go for python documaentation or source code to understand the implementation of the print function in Python.
# Like we take print in behind the scene, we saw in documatation 'docs.python.org/3/library/functions.html#print' that it is a,
...
# These documenation tell us about the parameters which are already defined in python. If we go beyond the parameters, 
...
# What does it mean? 
"""
# In case 1: we can see that first we write some str in palce of '*objects' and do change in the sep no space & remove the '\n',
# which means next line also remove from this.Now check output we can understand more.
# Case 2: we change nothing in *objects, just add what we remove i above case. Only change is seprator have '?',
# question mark at the place of space Now show the ouput/result.
# case 3: No changes. Same check the output.
# I use string for better understanding & clearify the concept of print function in python. We can use variable also in place of string. 
# Till now we don't change after 'end='\n''
# Now, If you write file=sys . we need to import sys for activate this function in our currnet code window.
# file we can write str and in stdout we can write variable & sys need to import a libaray for support the code
# flush is use to collect the all data and print.
# so, this expalin how our print function is work. or  we  can say behind the scenes of print function.
# Library are the pre-writen code by python, we just need to import them only.
"""
# use Case of print function according  to documataion.
import sys
str1 = "yes"
str2 = "no"
str3 = "maybe"
